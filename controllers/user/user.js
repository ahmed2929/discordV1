...
var acceptFriendRequest = async (req, res, next) => {
    ...
    try {
        ...
        const frequest = await fRequest.find({
            _id: requestId,
            to: req.userId
        })
...
var blockUser = async (req, res, next) => {
    try {
        ...
        if (!blockeduser) {
            Response.CustomResponse(res, 422, "user not found")
        }
...
var editProfile = async (req, res, next) => {
    try {
        ...
        if (!user) {
            Response.CustomResponse(res, 422, "error happened")
        }
        (await user).updateOne({name:name||user.name},{photo:photo||user.photo});
       (await user).save()
    } catch(err) {
        console.debug(err)
        if (!err.statusCode) {
